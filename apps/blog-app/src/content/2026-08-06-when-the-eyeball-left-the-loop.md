...
Agents made producing a plausible visual diff almost free. They did not make
**knowing whether it's right** any cheaper. So the bottleneck moved, and if your
...
Reinstate it deliberately, as a **named stage with numbers in it**: measure the
resolved computed style and the composited pixels against the design's full state
matrix. Then turn that stage into a reusable asset so it's cheap enough to actually
run, and so the agent can eventually run it for you.
...
the step back, explicitly and with instruments. Step through the three stages:
...
- Utility classes for mutually exclusive states compete on **specificity**, not on
  your intent. A hover-prefixed class outranks a plain one, so a "selected" state you
  added can silently never appear.
...
None of that is expressible in the artifacts CI inspects. Unit and component tests
...
1. **A green pipeline means *valid*, not *correct*.** For visual work those two are
   nearly unrelated. Say the quiet part out loud in your definition of done, or the
   green will keep standing in for a judgment nobody made.
...
4. **Specifications stop being absorbed and must be extracted.** A human
   ...
   pull the **state matrix** yourself — that's now an input you owe the process.
5. **Measure; don't eyeball.** Once verification is deliberate, make it numeric. A
   1px border and a three-parts-in-255 color delta are both invisible to the eye and
...
7. **Prefer additive changes to shared code over mutating them.** Adding a variant
...
8. **Reuse the near-miss token instead of minting a new primitive.** An existing
   semantic token already carries values for every theme; a new primitive obliges you
   to invent values for states the design never specified. Quantify the delta and let
   a human judge "imperceptible" — that beats "exact but under-specified."
...
10. **Branches are multi-writer now.** Humans and agents push to the same branch
    concurrently. Assume someone moved it while you worked, always push with a lease,
    and when the lease refuses, restack onto their work rather than overriding it.

## Approaches that work

**Interrogate the artifact, not the source.** Ask the running page what it actually
computed for the properties in the acceptance criterion. This is a few seconds of
work and it is the single highest-value check available — in the case behind this
note, one query would have returned a 1px border on a change whose whole purpose was
removing it.
...
state while breaking three others is the easy failure. Most design tools will hand
over the named tokens and per-state values if you ask for them structurally rather
than sampling a picture.
...
explicit precedence order rather than stacking conditional classes and hoping the
intended one wins. This converts a specificity bet into a pure function with one
...
- **Hovering a child also hovers its ancestor.** Measure a child's hover fill and
  ...
  the wrong element.
- **The agent's own summary is not evidence.** In the case behind this note, the
...
- [ ] Exercised the state that regressed; labelled any state you had to construct.
- [ ] Wrote down what you did **not** verify.
- [ ] Restacked on current trunk before measuring; pushed with a lease.
